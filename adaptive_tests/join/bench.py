...
def join_weld(values, adaptive, lazy, threads):
    ...
    conf.set("weld.llvm.optimization.level", "2")
...
# Program
N_vals = [50000, 1000000, 50000000]
s_hit_vals = np.linspace(0.1, 1.0, num=10)
t_hit = 0.7
u_hit = 0.55
trials = 5
types = ['Normal', 'Adaptive', 'Lazy']
threads_vals = [1, 16]

csv = 'type,n,s_hit,t_hit,u_hit,threads,comp_time,exec_time'
print csv
for N in N_vals:
    for s_hit in s_hit_vals:
        data = generate_data(N, s_hit, t_hit, u_hit)
        for t in types:
            ...
            for threads in threads_vals:
                ...
                for i in range(trials):
                    ...
                    row = '%s,%d,%f,%f,%f,%d,%f,%f'  % (t, N, s_hit, t_hit, u_hit, threads, comp_time, exec_time)
                    print row
                    csv = '%s\n%s' % (csv, row)
