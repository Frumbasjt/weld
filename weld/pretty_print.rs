    ...
    fn print(&self) -> String {
        match *self {
            ...
            Builder(BloomBuilder(ref elem), ref annotations) => {
                format!("{}bloombuilder[{}]", annotations, elem.print())
            }
            BloomFilter(ref t) => format!("bloomfilter[{}]", t.print())
        }
    }
}
...
fn print_expr_impl<T: PrintableType>(expr: &Expr<T>,
    ...
    let expr_str = match expr.kind {
        ...
        SimdWidth(t) => {
            format!("simdwidth({})", print_type(&Scalar(t)))
        }
        ...
        Keys { ref child_expr } => {
            format!("keys({})", print_expr_impl(child_expr, typed, indent, should_indent))
        }
        ...
        NewBuilder(ref args) => {
            match args.len() {
                0 => expr.ty.print(),
                _ => {
                    let args_str = join("(", ",", ")", args.iter().map(|e| print_expr_impl(e, typed, indent, should_indent)));
                    format!("{}{}", expr.ty.print(), args_str)
                }
            }
        }
        ...
        BloomFilterContains {
            ...
            format!("bfcontains({},{})", print_expr_impl(bf, typed, indent, should_indent), print_expr_impl(item, typed, indent, should_indent))
        }
    };
