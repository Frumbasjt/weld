...
use pretty_print::print_typed_expr;
...
pub fn infer_types(expr: &mut PartialExpr) -> WeldResult<()> {
    ...
    for (sym, value) in expr.annotations.run_vars().iter_mut() {
        try!(infer_up(value, &mut env));
...
fn has_all_types(expr: &PartialExpr) -> bool {
    ...
    expr.annotations.exprs().all(|c| has_all_types(c)) && expr.children().all(|c| has_all_types(c))
}
...
fn infer_up(expr: &mut PartialExpr, env: &mut TypeMap) -> WeldResult<bool> {
    ...
    for annot_expr in expr.annotations.exprs_mut() {
        changed |= try!(infer_up(annot_expr, env));
    }
...
fn infer_locally(expr: &mut PartialExpr, env: &mut TypeMap) -> WeldResult<bool> {
    match expr.kind {
        ...
        SimdWidth(_) => push_complete_type(&mut expr.ty, Scalar(I64), "SimdWidth"),
        ...
        Keys { ref mut child_expr } => {
            ...
            if let Dict(ref key_type, _) = child_expr.ty {
                changed |= push_type(&mut expr.ty, &Vector(key_type.clone()), "Keys")?;
            };
            Ok(changed)
        }
        ...
        StrSlice {
            ...
            if let Vector(_) = data.ty {
                let mut changed = false;
                changed |= try!(push_complete_type(&mut offset.ty, Scalar(I64), "StrSlice"));
                changed |= try!(push_type(&mut expr.ty, &data.ty, "StrSlice"));
                Ok(changed)
            } else {
            ...
            for func in fors {
                if let Lambda { ref mut body, .. } = func.kind {
                    if let For { ref mut builder, .. } = body.kind {
                        changed |= try!(push_type(&mut expr.ty, &builder.ty, "SwitchFor"));

                        let func_type = Function(vec![Scalar(I64), Scalar(I64)], Box::new(builder.ty.clone()));
                        changed |= try!(push_type(&mut func.ty, &func_type, "SwitchFor"));
                    }
                }
            }
            ...
            if let Builder(ref bty, _) = expr.ty {
                match *bty {
                    VecMerger(ref elem, _, _) => {
                        match args.len() {
                            1 => {
                                changed |= try!(push_type(&mut args[0].ty,
                                                          &Vector(elem.clone()),
                                                          "NewBuilder(VecMerger)"));
                            }
                            _ => {
                                return compile_err!("Expected single argument for NewBuilder of type \
                                                  VecMerger");
                            }
                        }
                    }
                    Merger(ref elem, _) => {
                        match args.len() {
                            ...
                            _ => {
                                return compile_err!("Expected 1 or 0 arguments for NewBuilder of type Merger");
                            }
                        }
                    }
                    Appender(_) => {
                        match args.len() {
                            ...
                            _ => {
                                return compile_err!("Expected 1 or 0 arguments for NewBuilder of type Appender)");
                            }
                        }
                    }
                    BloomBuilder(_) => {
                        match args.len() {
                            1 => {
                                changed |= try!(push_type(&mut args[0].ty, &Scalar(I64), "NewBuilder(BloomBuilder)"));
                            }
                            _ => {
                                return compile_err!("Expected 1 argument for NewBuilder of type BloomBuilder)");
                            }
                        }
                    }
                    _ => {}
        ...
        BloomFilterContains {
            ...
            if let BloomFilter(ref ty) = bf.ty {
                let mut changed = false;
                changed |= try!(push_type(&mut item.ty, &ty, "BloomFilterContains"));
                changed |= try!(push_complete_type(&mut expr.ty, Scalar(Bool), "BloomFilterContains"));
                Ok(changed)
...
fn push_type(dest: &mut PartialType, src: &PartialType, context: &str) -> WeldResult<bool> {
    ...
    match *dest {
        ...
        Builder(BloomBuilder(ref mut dest_elem), ref mut dest_annotations) => {
            match *src {
                Builder(BloomBuilder(ref src_elem), ref src_annotations) => {
                    let mut changed = push_type(dest_elem.as_mut(), src_elem.as_ref(), context)?;
                    if *dest_annotations != *src_annotations {
                        if !src_annotations.is_empty() {
                            *dest_annotations = src_annotations.clone();
                    ...
                    match **dest_elem {
                        Scalar(_) => {}
                        _ => { return compile_err!("Bloom filter builders only support scalars"); }
                    }
                    Ok(changed)
                }
                _ => compile_err!("Mismatched types in {}", context)
        ...
        BloomFilter(ref mut dest_elem) => {
            match *src {
                BloomFilter(ref src_elem) => {
                    let changed = push_type(dest_elem.as_mut(), src_elem.as_ref(), context)?;

                    match **dest_elem {
                        Scalar(_) => {}
                        _ => { return compile_err!("Bloom filters only support scalars"); }
                    }
                    Ok(changed)
                }
                _ => compile_err!("Mismatched types in {}", context)
